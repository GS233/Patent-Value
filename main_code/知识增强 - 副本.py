...
# 定义原始CSV文件和新CSV文件的路径
original_csv_file = './data/dev.csv'
new_csv_file = './data/dev_llm123.csv'
import time
...
with open(original_csv_file, 'r', encoding='utf-8') as original_file, \
    ...
    for row in tqdm(csv_reader):
        try:
            # 添加新的列数据
            print(row[0])
            ans,explaination,keywords_list,keywords_explaination_list = get_knowledge_enhance(row[0])
            row.append(ans)
            row.append(explaination)
            row.append(keywords_list)
            row.append(keywords_explaination_list)
